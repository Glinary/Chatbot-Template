...
from schema.bot_schema import BotSchema
...
class MongoTrackerStore(TrackerStore, SerializedTrackerAsText):
    ...
    async def save(self, tracker: DialogueStateTracker) -> None:
        ...
        for e in additional_events:
            if (e.as_dict().get("event") == "user"):
                format_user_event(e.as_dict())
                
            if (e.as_dict().get("event") == "bot"):
                format_bot_event(e.as_dict())
        #     logging.info(f'I GOT THIS: {e.as_dict().get("event")}')
...
def format_user_event(e):
    ...
    logging.info(f"USER_EVENT_JSON: {user_event}")

    db_user = UserSchema(user_event)
    db_user.store()

def format_bot_event(e):
    bot_event = {
        "event": e.get("event"),
        "timestamp": e.get("timestamp"),
        "text": e.get("text"),
    ...
    logging.info(f"BOT_EVENT_JSON: {bot_event}")

    db_bot = BotSchema(bot_event)
    db_bot.store()